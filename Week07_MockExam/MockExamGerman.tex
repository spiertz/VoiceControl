...
\end{centering}\\\\
\clearpage
Im Folgenden wird eine STFT auf ein Signal $x(n)$ angewendet. Die Abtastrate ist $r=48$ kHz, das Signal hat eine Dauer von $70$ Minuten. Die Blocklänge ist $N=1000$, die Überlappung ist $75$ \%, die Transformationslänge ist $K=2048$.
...
\question Aufgabe (10 Punkte):\\
\begin{parts}
\part Ein Signal hat die Frequenz $f=1234$ Hz. Berechnen Sie die dazugehörigen Werte in Bark und mel.\\\\
\begin{centering}
\begin{tikzpicture} %Bild 3
\draw[step =.5cm ,gray ,very  thin] (0,0)  grid(15 ,5);
\end{tikzpicture}
...
\question Aufgabe (10 Punkte):\\
\begin{parts}
\part Menschliche Sprache wird mit einem Mikrofon aufgenommen. Was ist der typische Frequenzbereich der menschlichen Sprache? Was ist die typische mittlere Amplitude im Zeitbereich des aufgenommenen Signals?\\\\
\begin{centering}
\begin{tikzpicture} %Bild 3
...
import numpy as np
\end{python}
beginnen. Bestimmen Sie für jeden Schnipsel, ob die Ausgabe A oder B ist (durch Markieren der korrekten Ausgabe auf dem Aufgabenblatt). Jede richtige Antwort gibt $2$ Teilpunkte. Die Gesamtpunktzahl für die gesamte Aufgabe berechnet sich aus der Summe der Teilpunkte minus zehn. Bei einer negativen Gesamtpunktzahl werden $0$ Punkte vergeben.
